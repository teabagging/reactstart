...
Along with the root component, you need to provide a list of {2}>bootstrap `<script>` paths. Your root component should return **the entire document including the root `<html>` tag.**
...
React will inject the [doctype](https://developer.mozilla.org/en-US/docs/Glossary/Doctype) and your {2}>bootstrap `<script>` tags into the resulting HTML stream:
