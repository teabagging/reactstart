...
1. The current state of this state variable, initially set to the {3}>initial state you provided.
2. The {2}>`dispatch` function that lets you change it in response to interaction.

To update what's on the screen, call {2}>`dispatch` with an object representing what the user did, called an *action*:
...
React will pass the current state and the action to your {4}>reducer function. Your reducer will calculate and return the next state. React will store that next state, render your component with it, and update the UI.
