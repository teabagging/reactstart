...
`use` returns the {2}>context value for the context you passed. To determine the context value, React searches the component tree and finds **the closest context provider above** for that particular context.
...
Unlike [`useContext`](/reference/react/useContext), {2}>`use` can be called in conditionals and loops like `if`.
...
{2}>`use` is called from inside a `if` statement, allowing you to conditionally read values from a Context.
...
Data can be streamed from the server to the client by passing a Promise as a prop from a Server Component to a {2}>Client Component.
...
The {2}>Client Component then takes {4}>the Promise it received as a prop and passes it to the {5}>`use` API. This allows the {2}>Client Component to read the value from {4}>the Promise that was initially created by the Server Component.
...
```
Because {2}>`Message` is wrapped in {3}>[`Suspense`](/reference/react/Suspense), the fallback will be displayed until the Promise is resolved. When the Promise is resolved, the value will be read by the {5}>`use` API and the {2}>`Message` component will replace the Suspense fallback.
...
To use the Promise's `catch` method, call `catch` on the Promise object. `catch` takes a single argument: a function that takes an error message as an argument. Whatever is {2}>returned by the function passed to `catch` will be used as the resolved value of the Promise.
