...
When `TeamReport` renders its list of `users` and reaches the same `user` object, it will call {2}>`getUserMetrics` and read the result from cache.
...
In the above example, {2}>`Precipitation` and `Temperature` each call `cache` to create a new memoized function with their own cache look-up. If both components render for the same `cityData`, they will do duplicate work to call `calculateWeekReport`.
...
```
Here, both components call the {3}>same memoized function exported from `./getWeekReport.js` to read and write to the same cache. 
...
If `AnimatedWeatherCard` and `MinimalWeatherCard` both render for the same city, they will receive the same snapshot of data from the {2}>memoized function. 

If `AnimatedWeatherCard` and `MinimalWeatherCard` supply different city arguments to {2}>`getTemperature`, then `fetchTemperature` will be called twice and each call site will receive different data.
...
{3}>Asynchronous rendering is only supported for Server Components.
...
When rendering `Profile`, we call {2}>`getUser` again. If the initial `getUser` call has already returned and cached the user data, when `Profile` {2}>asks and waits for this data, it can simply read from the cache without requiring another remote procedure call. If the  initial data request hasn't been completed, preloading data in this pattern reduces delay in data-fetching.
...
In calling {2}>`getData` the first time, the promise returned from `fetchData` is cached. Subsequent look-ups will then return the same promise.

Notice that the first {2}>`getData` call does not `await` whereas the {3}>second does. [`await`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/await) is a JavaScript operator that will wait and return the settled result of the promise. The first {2}>`getData` call simply initiates the `fetch` to cache the promise for the second {3}>`getData` to look-up.

If by the {3}>second call the promise is still _pending_, then `await` will pause for the result. The optimization is that while we wait on the `fetch`, React can continue with computational work, thus reducing the wait time for the {3}>second call. 
...
```
Re-writing the previous example to use `cache`, in this case the {3}>second instance of `WeatherReport` will be able to skip duplicate work and read from the same cache as the first `WeatherReport`. Another difference from the previous example is that `cache` is also recommended for {2}>memoizing data fetches, unlike `useMemo` which should only be used for computations.
